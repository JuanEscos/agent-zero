...
class Actor:
    ...
    def act_epsilon_greedy(self, st, epsilon):
        ...
        if self.cfg.noisy:
            ...
            action = [
                act_greedy if p > epsilon else act_random
                for p, act_random, act_greedy in zip(
                    np.random.rand(self.cfg.num_envs), action_random, action_greedy
                )
            ]
    ...
    def sample(
        self, steps, epsilon, model, testing=False, test_episodes=20, render=False
    ):
        self.model = model
        ...
        while True:
            ...
            if not testing:
                ...
                for st, at, rt, dt, st_next, inf in zip(
                    self.obs, action, reward, done, obs_next, info
                ):
            ...
            if testing and (
                len(rs) > test_episodes or step > self.cfg.max_record_ep_len
            ):
                break
